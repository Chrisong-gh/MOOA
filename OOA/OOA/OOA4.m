...
%                 r2=K(randi([1 numel(K)]));
                ...
                Octopus(i).Tgroup(j).pos= Octopus(i).Tgroup(j).pos+rand()*(pBest-Octopus(i).Tgroup(index).pos).*Levy(dim); %
...
%                 end
                ...
                Octopus(index(flagindex)).Tgroup(i).pos=-ones(1,dim).*(Octopus(index(flagindex)).pos-ll)+rand(1,dim).*(Octopus(index(flagindex)).pos+ll);%
                % Octopus(index(flagindex)).Tgroup(i).pos=(2*rand(1,dim)-1)*ll+Octopus(index(flagindex)).pos;
