...
function [pBestScore,pBest,Convergence_curve]=OOA2(N,T,lb,ub,dim,fobj)
...
%                 r1 = floor(N*rand()+1);  % 0-30 
                ...
                Octopus(i).Tgroup(j).pos= Octopus(i).Tgroup(j).pos+rand()*(SP.pos-Octopus(i).Tgroup(r2).pos).*Levy(dim); %
...
%                 Octopus(i).Tgroup(j).pos=Octopus(i).Tgroup(j).pos+rand()*(SP.pos-Octopus(i).Tgroup(j).pos).*Levy(dim);
                Octopus(i).Tgroup(j).pos=SP.pos+rand()*(SP.pos-Octopus(i).Tgroup(j).pos).*Levy(dim);
                Octopus(i).Tgroup(j).cost=fobj(Octopus(i).Tgroup(j).pos);
            end 
