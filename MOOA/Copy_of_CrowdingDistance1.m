function CrowdDis = CrowdingDistance1(PopObj,FrontNo)
...
%--------------------------------------------------------------------------
    ...
    for f = 1 : length(Fronts)
        ...
        for i = 1 : M
            ...
            if length(Rank)>1
                CrowdDis(Front(Rank(1))) = CrowdDis(Front(Rank(1)))+PopObj(Front(Rank(2)),i)/Fmax(i);
            else
                CrowdDis(Front(Rank(1))) = CrowdDis(Front(Rank(1)))+PopObj(Front(Rank(1)),i)/Fmax(i);
            end

            
            for j = 2 : length(Front)
                if j== length(Front)
                    CrowdDis(Front(Rank(j))) = CrowdDis(Front(Rank(j)))+(PopObj(Front(Rank(j)),i)-PopObj(Front(Rank(j-1)),i))/Fmax(i);
                else
                    CrowdDis(Front(Rank(j))) = CrowdDis(Front(Rank(j)))+(PopObj(Front(Rank(j+1)),i)-PopObj(Front(Rank(j-1)),i))/Fmax(i);
                end
